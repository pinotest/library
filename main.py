...
from EpisodeMovie import EpisodeMovie
import random
...
def get_movies():
    ...
    '''
    library_movies = [j for i, j in enumerate(
        library) if (type(library[i]) == BaseMovie)]
...
def get_series():
    ...
    '''
    library_series = [j for i, j in enumerate(
        library) if (type(library[i]) == SeriesMovie)]
...
def search(title):
    ...
    '''
    library_search = [j for i, j in enumerate(library) if (
        library[i].title.upper() == title.upper())]
...
def top_titles(top_number, content_type=2):
    ...
    if content_type == 0:
        ...
        print("Najpopularniejsze filmy dnia ",
              datetime.strftime(datetime.now(), '%d:%m:%Y'))
    elif content_type == 1:
        print("Najpopularniejsze seriale dnia ",
              datetime.strftime(datetime.now(), '%d:%m:%Y'))
    ...
    sorted_library = sorted(
        temp_library, key=lambda Movie: Movie.number_plays, reverse=True)
    top_library = []
...
def add_full_season_series(title, initial_release, content_type, season_number, episode_numbers):
    ...
    #     library.append(SeriesMovie(title=title, initial_release=initial_release,
    #                                content_type=content_type, season_number=season_number, episode_number=i))
    for i in range(1, episode_numbers+1):
        library.append(EpisodeMovie(title=title, initial_release=initial_release,
                                    content_type=content_type, season_number=season_number, episode_number=i))
...
library.append(SeriesMovie(title="Dark", initial_release=2018,
                           content_type='drama', season_number=1))
library.append(SeriesMovie(title="Dark2", initial_release=2018,
                           content_type='drama', season_number=1))
library.append(SeriesMovie(title="Dark3", initial_release=2018,
                           content_type='drama', season_number=1))
